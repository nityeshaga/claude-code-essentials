...
| A timer, polling loop, or manual listener started in `connect()` with no `disconnect()` teardown | `connect()`/`disconnect()` fire every time the element enters/leaves the document; an unreleased resource outlives its element and keeps working in the background | Pair acquire/release: anything acquired in `connect()` is released in `disconnect()` → `07-stimulus-widgets.md` |
...
| JS measuring/computing what CSS can derive; a bespoke JS formatting vocabulary | A second layout engine in JS that the stylesheet already has; a vocabulary the rest of the app doesn't share | One synced `data-*` attribute + CSS (the DOM attribute is the state); port the Rails helper (`toSentence`) instead of inventing → `07-stimulus-widgets.md` |
