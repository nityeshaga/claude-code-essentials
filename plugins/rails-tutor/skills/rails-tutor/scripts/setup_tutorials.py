#!/usr/bin/env python3
"""
Set up the central tutorials repository for rails-tutor.

Usage:
    python setup_tutorials.py
    python setup_tutorials.py --create-github-repo

Creates ../rails-tutor-tutorials/ if it doesn't exist, initializes git,
...
README_CONTENT = """# Rails Tutor - My Learning Journey

This repository contains my personalized Rails tutorials created with the [rails-tutor](https://github.com/nityeshaga/claude-code-essentials) Claude Code plugin.
...
- `concepts`: What Rails concepts are covered
...
def setup_tutorials_repo(create_github=False):
    ...
    try:
        ...
        subprocess.run(
            ['git', 'commit', '-m', 'Initial commit: Rails learning journey'],
        ...
        if create_github:
            result = subprocess.run(
                ['gh', 'repo', 'create', 'rails-tutor-tutorials', '--private', '--source=.', '--push'],
...
def main():
    parser = argparse.ArgumentParser(
        description="Set up the central tutorials repository for rails-tutor"
    )
    parser.add_argument(
        "--create-github-repo",
        action="store_true",
