...
It returns `{ cloneRoot, crux, chadReport, proposals, imageFindings, plan }` — `plan.decisions` (per-rewrite verdict + confidence), `plan.addBackMenu`, and `imageFindings` (Chad's verdict on each flagged image, with either a `remadePath` — an updated image the defender produced in the clone — or a `plan` to update it). Hand that to a fresh agent to apply as a PR; don't burn your own context materializing edits.
