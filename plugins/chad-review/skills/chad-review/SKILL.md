...
description: >-
  ...
  each span to the job or rewrites it plainer. This goes for two rounds. Use to remove instances of 
...
- **The Chad pass** — Chad meets each file cold and asks the dumbest honest questions he has (prompt below). For **images**, Chad looks at the picture and a defender rules whether it earns its place — a verdict (keep / remake / cut) plus concrete guidance, not a rewrite. The defender ties each span to the crux, or remakes it — cut, plainer, or restructured to reach the point faster. The defender gets the last word: Chad is doing it a service, and the defender owns the improved copy. Then it runs **again**: a *fresh* Chad agent — no memory of the first round — cold-reads the rewrite, and the defender gets one more pass to fix whatever he's still unimpressed by. This second round is where most residue dies — the rewrite is a new generation, so it can smuggle in fresh showing-off the first Chad never saw. Capped at **two rounds** so the polish can't loop forever; anything the defender still argues against in that final pass is surfaced to the human, not chased. Each round, after the questions, Chad also steps back and gives one blunt **bird's-eye conclusion** on the whole thing — is it too long, too busy, in the wrong shape, or does it land — the overall take that local questions might miss.
...
It returns `{ cloneRoot, crux, chadReport, proposals, imageFindings, plan }` — `plan.decisions` (per-rewrite verdict + confidence), `plan.addBackMenu`, and `imageFindings` (Chad's verdict + guidance on each user-facing image he flagged). Hand that to a fresh agent to apply as a PR; don't burn your own context materializing edits.
